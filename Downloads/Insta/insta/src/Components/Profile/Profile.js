...
const [posts,setPosts] = useState([]);
  ...
  const toggleFollow = async () => {
    try {
      ...
      setIsFollowing(followdata.data.follow);
      setFollowersCount(followdata.data.followedBy);
    } catch (err) {
      console.log(err);
    }
  }
              ...
              <div className="flex items-center space-x-4 mb-5">
                ...
                {profileData.isOwnProfile ? (
                  ...
                  <div className="flex space-x-2">
                    <button 
                      onClick={toggleFollow}
                      className={`font-semibold py-1.5 px-6 rounded-lg text-sm transition ${
                        isFollowing 
          ...
          <div className="flex justify-around md:justify-center md:space-x-20">
            {tabs.map(tab => (
              <button
                ...
                className={`flex items-center space-x-1 py-3 px-4 border-t-[1px] -mt-[1px] transition ${
                  activeTab === tab.id 
